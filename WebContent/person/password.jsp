	...
	<div class="center">
		<div class="col-main">
			<div class="main-wrap">
				...
				<form class="am-form am-form-horizontal" m action="<%=basePath%>/InformationServlet?type=pwd">
					<div class="am-form-group">
						<label for="user-old-password" class="am-form-label">原密码</label>
						<div class="am-form-content">
							<input type="password" id="user-old-password"
								placeholder="请输入原登录密码" name = "former_pwd">
						</div>
					</div>
					<div class="am-form-group">
						...
						<div class="am-form-content">
							<input type="password" id="user-new-password"
								placeholder="由数字、字母组合" name="one_pwd">
						</div>
					</div>
					<div class="am-form-group">
						...
						<div class="am-form-content">
							<input type="password" id="user-confirm-password"
								placeholder="请再次输入上面的密码" name="two_pwd">
						</div>
					</div>
					<div class="info-btn">
						<input type="submit" class="am-btn am-btn-danger" value="保存修改" id="bc">
					</div>
				  ...
				  $(function(){
					  ...
					  $.each($("input"),function(i,dom){
						  $(dom).blur(function() {
							  ...
							  if($(dom).attr('name')=="former_pwd"){//原密码
							  $(".msgpwd").remove();
							  var former_pwd = $("#user-old-password").val();
								  if(former_pwd == "" || former_pwd.trim() == "") {
										msg = "<span class='msgpwd' style='color:red;'>原密码不能为空</span>";
										$("#bc").attr("disabled","");
										
									} else if("${user.pwd}" != former_pwd) {
										msg = "<span class='msgpwd' style='color:red;'>原密码错误</span>";
										$("#bc").attr("disabled","");
										
									} else {
										msg = "<span class='msgpwd' style='color:green;'>ok</span>";
									}
								  $(dom).parent().append(msg);
							  }else if ($(dom).attr('name') == "one_pwd") {//新密码
								$(".msgpwd1").remove();
							  var one_pwd = $("#user-new-password").val();
							  var pPattern = /^[a-z0-9_-]{6,18}$/;
							  if(one_pwd == "" || one_pwd.trim() == "") {
									msg = "<span class='msgpwd1' style='color:red;'>新密码不能为空</span>";
									$("#bc").attr("disabled","");
								} else if(!pPattern.test(one_pwd)) {
									msg = "<span class='msgpwd1' style='color:red;'>密码格式错误</span>";
									$("#bc").attr("disabled","");
								} else {
									msg = "<span class='msgpwd1' style='color:green;'>ok</span>";
								}
								$(dom).parent().append(msg);
							}else if($(dom).attr('name') =="two_pwd"){//再次输入
								//先清除
								$(".msgpwd2").remove();
								var one_pwd = $("#user-new-password").val();
							    var two_pwd= $("#user-confirm-password").val();
								if(two_pwd == "" || two_pwd.trim() == "") {
									msg = "<span class='msgpwd2' style='color:red;'>再次输入不能为空</span>";
									$("#bc").attr("disabled","");
								} else if(two_pwd != one_pwd) {
									msg = "<span class='msgpwd2' style='color:red;'>再次输入的密码与新密码不匹配</span>";
									$("#bc").attr("disabled","");
								} else {
									msg = "<span class='msgpwd2' style='color:green;'>ok</span>";
								}
								$(dom).parent().append(msg);
							}
						});
